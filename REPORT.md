...
Six map-identity collision groups demonstrated why the first-match approach was
not sufficient. Some containers have identical program maps, and some also have
identical sequence evidence. The final resolver therefore combines:

- exact live program-map evidence;
- live sequence-table evidence;
- file-load provenance captured from the game's own container access.
...
The PC engine remains the source of live interactive sequence events. The hook
binds those events to the exact locally extracted Dreamcast container data and
renders them through the Dreamcast Manatee/AICA path based on the GPLv3 Highly
Theoretical core. Extracted Dreamcast DSEQ data is currently used as container
identity evidence; the hook does not independently schedule DSEQ playback.
Consequently, the module restores synthesis, driver, allocation, maps and sample
banks, but would not by itself correct a genuinely different PC scene-to-cue
...
- music, ambience and their Dreamcast DSP path: restored renderer;
- ordinary PC sound effects: native PC path;
- movie audio: native Bink path;
- game timing and event selection: native game logic.
...
- Music uses the PC engine's live cue/event timing. A genuinely different PC
  scene-to-cue decision is not independently replaced by Dreamcast DSEQ
  scheduling in the current version.
- One ambiguous persistent music state was deliberately left unbound in the
  latest retained startup log; its full-game relevance remains to be classified.
