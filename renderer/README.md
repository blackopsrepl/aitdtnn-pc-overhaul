...
- Frame pacing enabled by default: gameplay is capped at 60 FPS and cinematics
  ...
  untouched because the engine already paces movie frames through `BinkWait`.
  Configure it in `renderer\aitd4-overhaul.ini` under `[FrameRate]`;
  `GameLimit`/`MovieLimit` accept a frame rate, `0` for uncapped, or `Auto` for
  the movie's authored rate.
